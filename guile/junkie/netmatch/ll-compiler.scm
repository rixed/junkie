...
(define (stub->so stub)
  (let* ((srcname     (string-copy "/tmp/netmatch-ll.c.XXXXXX"))
    ...
    (let* ((cc       (or (getenv "NETMATCH_CC")       build-cc))
           ...
           (ldplugin (or (getenv "NETMATCH_LDFLAGS_PLUGIN") build-ldflags-plugin))
           (ext      (or (getenv "NETMATCH_PLUGIN_EXT") plugin-ext))
           (libname  (string-append srcname "." ext))
           (cmd      (string-append cc " " cppflags " " cflags " " ldflags " " ldplugin " -o " libname " -xc " srcname))
           (status   (system cmd)))
