...
(define (expr->stub proto expr expected-type)
  (let ((perform-op (lambda (op-name params)
        ...
        (cst->stub  (lambda x ; given a proto + symbol, return the expression that alias it, or #f if unknown (and then the symbol is a field name)
                      (match x
                             (('arp 'request) 1)
                             (('arp 'reply) 2)
                             (('dns 'unset) 0)
      ...
      ((symbol? expr)
       ...
       (let* ((str (symbol->string expr)))
         (or (and (regname? str)
             ...
             (and=> (cst->stub proto expr) (lambda (x) (expr->stub proto x expected-type)))
