...
(define-syntax slog
  (syntax-rules ()
    ((_ lvl fmt ...) (let* ((loc   (current-source-location))
                            ...
                            (prettify (lambda (file)
                                        (if (string? file)
                                            (basename file)
                                            (object->string file))))
                            (file  (or (and=> (assq-ref loc 'filename) prettify) "<some file>"))
                            (func  (or (and=> (frame-procedure-name (stack-ref stack 1)) symbol->string) "")))
...
(define*-public (start-repl-server #:key (port 29000))
  (make-thread (lambda ()
                 (set-thread-name "J-repl-server")
                 (start-server (inet-pton AF_INET "127.0.0.1") port))))
...
(define-public (array-mem-stats)
  (let* ((tot-used-bytes     0)
         ...
         (stats              (filter-map (lambda (n)
                                           (let* ((s              (array-stats n)))
                                             (if (> (assq-ref s 'num-malloced) 0)
                                                 (let* ((used-bytes     (* (- (assq-ref s 'num-used)
                                                                              (assq-ref s 'num-holes))
                                                                           (assq-ref s 'entry-size)))
                                                        (malloced-bytes (* (assq-ref s 'num-malloced)
                                                                           (assq-ref s 'entry-size)))
...
(define-public (make-mux-hash-controller coll-avg-min coll-avg-max h-size-min h-size-max)
  (lambda (proto)
    (let* ((stats    (mux-stats proto))
           ...
           (colls    (assq-ref stats 'num-collisions))
           (lookups  (assq-ref stats 'num-lookups))
...
(define-public (report-cnx period)
  (let ((thread (lambda (period)
                  ...
                  (let ((max-tcp 0)
                        (max-udp 0))
                    (let loop ()
                      (let ((cur-tcp (assq-ref (proto-stats "TCP") 'num-parsers))
                            (cur-udp (assq-ref (proto-stats "UDP") 'num-parsers)))
