             ...
             [("duplicogram" "home")
              ...
              (let* ((width        900)
                ...
                (for-each (lambda (pt)
                          ...
                          pts)
                (set! tick-every-x (floor (/ max-dt nb-ticks-x)))
                (set! tick-every-y (floor (/ max-dups nb-ticks-y)))
                (slog log-debug "ticks: every ~s on X and every ~s on Y" tick-every-x tick-every-y)
                (respond
                  `((form
                      (label
                        "bucket width:"
                                         ...
                                         (let ((x (exact->inexact (dt->x t))))
                                           `(g (@ (class . "tick tick-x"))
                                               ...
                                               (text (@ (x . ,(exact->inexact (- x (/ font-width 2))))
                                                        ...
                                                        (font-size . ,font-width)
                                                        (text-anchor . "start"))
                                                     ,(exact->inexact (/ (* bucket-width t) 1000))))))
