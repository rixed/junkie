...
;;; Cedric Cellier guilified it, for the public domain.
...
(define-syntax let*-optionals:aux
  (syntax-rules ()
    ((aux ?orig-args-var ?args-var () ?body1 ?body2 ...)
     (if (null? ?args-var)
         (let () ?body1 ?body2 ...)
         (throw "too many arguments" (length ?orig-args-var)
                ?orig-args-var)))
...
(define (check-indices vec start start-name end end-name callee)
  (let ((lose (lambda things
                (apply throw "vector range out of bounds"
                       (append things
                               `(vector was ,vec)
...
(export vector-unfold-right)
(export vector-copy)
(export vector-reverse-copy)
(export vector-append)
(export vector-concatenate)
...
(export vector-index)
(export vector-index-right)
(export vector-skip)
(export vector-skip-right)
      ...
      (cond ((and (eq? target source)
                  (or (between? sstart tstart send)
                      (between? tstart sstart
                                (+ tstart (- send sstart)))))
               (throw "vector range for self-copying overlaps"
                      vector-reverse-copy!
                      `(vector was ,target)
                      `(tstart was ,tstart)
                      `(sstart was ,sstart)
